...
export async function isDockerBuildXInstalled(exec) {
    const { exitCode } = await exec('docker', ['buildx', '--help'], { silent: true });
    return exitCode === 0;
}
export async function buildImage(exec, imageName, imageTag, checkoutPath, subFolder, skipContainerUserIdUpdate, cacheFrom) {
    const folder = path.join(checkoutPath, subFolder);
    const devcontainerJsonPath = path.join(folder, '.devcontainer/devcontainer.json');
...
export async function runContainer(exec, imageName, imageTag, checkoutPath, subFolder, command, envs, mounts) {
...
export async function pushImage(exec, imageName, imageTag) {
    const args = ['push'];
    args.push(`${imageName}:${imageTag ?? 'latest'}`);
    const { exitCode } = await exec('docker', args, {});
    if (exitCode !== 0) {
...
export function parseMount(mountString) {
